...
export function Home() {
  ...
  const handleAssignFamily = (type: string, familyId: number | null) => {
    ...
    setMetas((prev) => prev.map((m) => (m.type === type ? { ...m, family_id: familyId } : m)));
  };
  ...
  return (
    <div className="fr-container fr-py-6w">
        ...
        {FAMILIES.map((family) => {
          ...
          return (
            <div key={family.id} className="fr-col-12 fr-col-sm-6 fr-col-md-4">
              <div
                onClick={() => {
                  if (familyMetas.length === 1) navigate(`/classifications/${encodeURIComponent(familyMetas[0].type)}`);
                  else if (familyMetas.length > 1) navigate('/classifications');
                }}
                style={{
                ...
                onMouseEnter={(e) => { if (familyMetas.length > 0) (e.currentTarget as HTMLDivElement).style.boxShadow = '0 4px 12px rgba(0,0,0,0.15)'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = 'none'; }}
              >
                <span style={{ color: family.color, fontWeight: 700, fontSize: '0.8rem' }}>#{family.id}</span>
                <span style={{ fontWeight: 600, color: family.color, fontSize: '1rem', lineHeight: 1.3 }}>{family.name}</span>
                {familyMetas.length > 0 && (
                  <span style={{ background: family.color, color: '#fff', borderRadius: '12px', padding: '2px 10px', fontSize: '0.75rem', marginTop: '4px' }}>
        ...
        <>
          ...
          <div className="fr-grid-row fr-grid-row--gutters">
            {metas.map((meta) => {
              ...
              const r = family ? parseInt(family.color.slice(1,3),16) : 0;
              const g = family ? parseInt(family.color.slice(3,5),16) : 0;
              const b = family ? parseInt(family.color.slice(5,7),16) : 0;
              return (
                <div key={meta.type} className="fr-col-12 fr-col-md-4">
                  <div style={{ border: `1px solid ${family ? family.color : 'var(--border-default-grey)'}`, borderRadius: '8px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontWeight: 600 }}>{meta.type}</span>
                      {family && (
                        <span style={{ background: `rgba(${r},${g},${b},0.15)`, color: family.color, borderRadius: '12px', padding: '2px 8px', fontSize: '0.7rem', border: `1px solid ${family.color}` }}>
                          {family.name}
                        </span>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      <Button size="small" priority="primary" onClick={() => navigate(`/classifications/${encodeURIComponent(meta.type)}`)}>
                        Ouvrir
                      </Button>
                      <select
      ...
      {!loading && metas.length === 0 && (
        <p className="fr-text--sm fr-text--mention">Aucune classification chargée. Importez un fichier JSON pour commencer.</p>
      )}
    </div>
  );
}
