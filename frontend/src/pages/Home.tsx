        ...
        {FAMILIES.map((family) => {
          ...
          return (
            <div key={family.id} className="fr-col-12 fr-col-sm-6 fr-col-md-4">
              ...
              >
                <span style={{ color: family.color, fontWeight: 900, fontSize: '1rem' }}>#{family.id}</span>
                <span style={{ fontWeight: 600, color: family.color, fontSize: '1.05rem', lineHeight: 1.3 }}>{family.name}</span>
                {familyMetas.length > 0 && (
                  <span className="fr-badge fr-badge--sm" style={{
                    background: family.color, color: '#fff', borderRadius: '12px',
                    padding: '2px 10px', fontSize: '0.75rem', marginTop: '2px',
                  }}>
