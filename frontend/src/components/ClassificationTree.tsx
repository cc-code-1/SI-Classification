...
function AddChildForm({ parentCode, type, onAdded, onCancel }: AddChildFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!code || !nom || !definition) { setError('Tous les champs sont requis.'); return; }
    try {
  ...
  return (
    <form onSubmit={handleSubmit} className="fr-p-2w fr-mt-1w" style={{ background: 'var(--grey-1000)', borderLeft: '3px solid var(--blue-france-sun-113)' }}>
...
function TreeNodeRow({ node, type, onSelectEntry, selectedCode, collapsed, onToggle, forceExpand, onRefresh, familyColor }: TreeNodeRowProps) {
    ...
    <li style={{ listStyle: 'none' }}>
      ...
      >
        {hasChildren ? (
          ...
          >
            <span className={expanded ? 'fr-icon-arrow-down-s-line' : 'fr-icon-arrow-right-s-line'} aria-hidden="true" />
          </button>
        ) : (
          <span style={{ width: '32px', display: 'inline-block' }} />
        )}
        {node.level > 0 && <span style={{ display: 'inline-block', width: `${node.level * 12}px` }} />}
      ...
      {showAddForm && (
        <AddChildForm parentCode={node.code} type={type} onAdded={() => { setShowAddForm(false); onRefresh?.(); }} onCancel={() => setShowAddForm(false)} />
      )}
      {hasChildren && expanded && (
        <ul style={{ paddingLeft: '16px', margin: 0 }}>
          {node.children.map((child) => (
            <TreeNodeRow key={child.code} node={child} type={type} onSelectEntry={onSelectEntry} selectedCode={selectedCode} collapsed={collapsed} onToggle={onToggle} forceExpand={forceExpand} onRefresh={onRefresh} familyColor={familyColor} />
          ))}
        </ul>
      )}
    </li>
...
function filterTree(nodes: ClassificationTreeNode[], query: string): { filtered: ClassificationTreeNode[]; count: number } {
  ...
  const recurse = (list: ClassificationTreeNode[]): ClassificationTreeNode[] => {
    ...
    for (const node of list) {
      const selfMatch = node.code.toLowerCase().includes(q) || node.nom.toLowerCase().includes(q) || node.definition.toLowerCase().includes(q) || node.annotations.some((a) => a.toLowerCase().includes(q));
  ...
  return { filtered: recurse(nodes), count };
}
...
function buildNodeMap(nodes: ClassificationTreeNode[]): Map<string, ClassificationTreeNode> {
  ...
  const recurse = (list: ClassificationTreeNode[]) => { for (const node of list) { map.set(node.code, node); recurse(node.children); } };
  recurse(nodes);
  return map;
}
...
export function ClassificationTree({ nodes, type, onRefresh, familyColor = '#000091' }: ClassificationTreeProps) {
  ...
  if (nodes.length === 0) return <p className="fr-text--sm fr-text--mention">Aucune entrée dans cette classification.</p>;
  ...
  const handleToggle = (code: string) => {
    setCollapsed((prev) => { const next = new Set(prev); if (next.has(code)) next.delete(code); else next.add(code); return next; });
  };
  ...
  return (
    <div className="fr-grid-row fr-grid-row--gutters">
      <div className="fr-col-12 fr-col-md-7">
        <div className="fr-text--sm fr-p-2w fr-mb-2w" style={{ background: 'var(--grey-975)', borderRadius: '4px' }}>
        ...
        <div className="fr-btns-group fr-btns-group--inline fr-mb-2w">
          <Button size="small" priority="tertiary" iconId="fr-icon-arrow-down-s-line" onClick={() => setCollapsed(new Set())}>Tout déplier</Button>
          <Button size="small" priority="tertiary" iconId="fr-icon-arrow-right-s-line" onClick={() => setCollapsed(new Set(collectCodesWithChildren(nodes)))}>Tout replier</Button>
        </div>
        <div className="fr-search-bar fr-mb-2w" role="search">
          <label className="fr-label" htmlFor="tree-search">Rechercher</label>
          <input className="fr-input" id="tree-search" type="search" placeholder="Code, nom, définition ou annotation…" value={search} onChange={(e) => setSearch(e.target.value)} />
          <button className="fr-btn" title="Rechercher" type="button">Rechercher</button>
        </div>
        {search.trim() && <p className="fr-text--xs fr-mb-1w" style={{ color: 'var(--text-mention-grey)' }}>{count} entrée(s) correspondante(s)</p>}
        <ul style={{ padding: 0, margin: 0 }}>
          {filtered.map((node) => (
            <TreeNodeRow key={node.code} node={node} type={type} onSelectEntry={setSelectedNode} selectedCode={selectedNode?.code ?? null} collapsed={collapsed} onToggle={handleToggle} forceExpand={forceExpand} onRefresh={onRefresh} familyColor={familyColor} />
          ))}
        </ul>
      </div>
      <div className="fr-col-12 fr-col-md-5">
        {selectedNode ? (
          <ClassificationCard
            ...
            type={type}
            ancestors={ancestors}
            onUpdated={(updated) => { setSelectedNode({ ...selectedNode, ...updated, children: selectedNode.children, level: selectedNode.level }); onRefresh?.(); }}
            onDeleted={() => { setSelectedNode(null); onRefresh?.(); }}
          />
        ) : (
          <div className="fr-p-3w" style={{ background: 'var(--grey-975)', borderRadius: '8px' }}>
            <p className="fr-text--sm fr-text--mention">Sélectionnez une entrée dans l'arbre pour afficher ses détails.</p>
          </div>
        )}
      </div>
    </div>
